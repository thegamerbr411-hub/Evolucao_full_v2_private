#!/usr/bin/env node
/** PR #53 — Visual QA V5 (emulator-5554 ONLY, auth resume + summary capture) */
...
const ROOT = path.join(process.cwd(), '.qa_runtime/visual_audit/hevy_ux_lote_c_v5');
...
function rawUiDump() {
  sleep(450);
  const remote = `/sdcard/hulc5_${Date.now()}.xml`;
...
function dismissDevMenu() {
  const xml = rawUiDump();
  if (/React Native Dev Menu|Connect to the bundler|Toggle Element Inspector/i.test(xml)) {
...
function findResourceBounds(xml, id) {
  ...
  const patterns = [
    new RegExp(`resource-id="[^"]*${esc}"[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`, 'i'),
    new RegExp(`content-desc="[^"]*${esc}[^"]*"[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`, 'i'),
  ];
...
function findNodeByNeedle(xml, needles, clickableOnly = false) {
  const nodes = String(xml || '').match(/<node[^>]+\/?>/gi) || [];
  for (const needle of needles) {
    if (!needle) continue;
    for (const node of nodes) {
      if (!node.includes(needle)) continue;
      if (clickableOnly && !/clickable="true"/i.test(node)) continue;
      const m = node.match(/bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/i);
      if (m) return { node, ...boundsCenter(`[${m[1]},${m[2]}][${m[3]},${m[4]}]`) };
    }
  }
  return null;
}

function tapNeedle(needles, xmlIn, clickableOnly = true) {
  const hit = findNodeByNeedle(xmlIn || uiDump(), needles, clickableOnly);
  if (!hit) return false;
  tap(hit.x, hit.y);
  return true;
}
...
## Depois do login
Rodar:
node .qa_runtime/scripts/hevy_ux_lote_c_sandbox_v5.cjs --resume-after-login
`;
...
function ensureOnWorkoutScreen() {
  for (let i = 0; i < 6; i++) {
    let xml = uiDump();
    if (xmlHas(xml, 'screen-workout')) return true;
    if (/CONTINUAR TREINO|Continuar treino/i.test(xml)) {
      tapResource('btn_home_main_cta', xml)
        || tapTextContains('CONTINUAR TREINO', xml)
        || tapTextContains('Continuar treino', xml);
      sleep(2500);
      continue;
    }
    if (openGuidedWorkout()) return true;
    sleep(1000);
  }
...
function ensureSimpleMode() {
  for (let i = 0; i < 10; i++) {
    let xml = uiDump();
    if (/workout-exercise-list-simple|workout-mode-simple-active|Treino guiado/i.test(xml)) return true;
    if (!/workout-exercise-list-advanced|Vis.o completa|Visão completa/i.test(xml)) {
      if (/input-weight/i.test(xml) && !/workout-exercise-list-advanced/i.test(xml)) return true;
      return false;
    }
    for (let s = 0; s < 5; s++) scrollUp();
    xml = uiDump();
    tapNeedle(['btn-toggle-workout-mode', 'Trocar'], xml)
      || tapResource('btn-toggle-workout-mode', xml)
      || tapPartialResource('btn-toggle-workout-mode', xml);
    sleep(1500);
  }
  const finalXml = uiDump();
  return /workout-exercise-list-simple|Treino guiado/i.test(finalXml);
}

function dismissInlineKeypad() {
  const xml = uiDump();
  if (/keypad-confirm|keypad-digit/i.test(xml)) {
    tapNeedle(['keypad-confirm', 'OK'], xml) || tapTextContains('OK', xml);
    sleep(500);
    return true;
  }
  return false;
...
function fillWorkoutFieldViaKeypad(fieldNeedle, value) {
  let xml = uiDump();
  tapNeedle([fieldNeedle, 'input-weight', 'input-reps'], xml)
    || tapResource(fieldNeedle, xml)
    || tapFirstPartial(fieldNeedle, xml);
  sleep(700);
  for (const ch of String(value).split('')) {
    xml = uiDump();
    const digitId = ch === '.' ? 'keypad-digit-dot' : `keypad-digit-${ch}`;
    tapNeedle([digitId, ch], xml) || tapTextContains(ch, xml);
    sleep(250);
  }
  xml = uiDump();
  tapNeedle(['keypad-confirm', 'OK'], xml) || tapTextContains('OK', xml);
...
function focusPendingSetRow() {
  const xml = uiDump();
  if (/Toque para focar/i.test(xml)) {
    tapTextContains('Toque para focar', xml);
    sleep(1000);
    return true;
  }
  if (tapTextContains('Pendente', xml)) {
    sleep(800);
    return true;
  }
  return false;
...
function openGuidedWorkout() {
  ...
  let xml = uiDump();
  if (xmlHas(xml, 'screen-workout')) return true;
  tapResource('btn_start_workout', xml) || tapResource('btn_home_main_cta', xml)
    || tapTextContains('CONTINUAR TREINO', xml) || tapTextContains('INICIAR TREINO', xml);
  sleep(2500);
  xml = uiDump();
  if (xmlHas(xml, 'screen-workout')) return true;
...
function fillWeightReps() {
  ...
  if (/Registrar \d+kg/i.test(xml)) {
    tapTextContains('Registrar', xml);
    sleep(1200);
    dismissInlineKeypad();
    return;
  }
  if (/input-weight/i.test(xml)) fillWorkoutFieldViaKeypad('input-weight', '10');
  xml = uiDump();
  if (/input-reps/i.test(xml)) fillWorkoutFieldViaKeypad('input-reps', '10');
  dismissInlineKeypad();
}
...
function tapSaveAction(xmlIn) {
  const xml = xmlIn || uiDump();
  return tapNeedle(['btn-save-set', 'btn-salvar-serie'], xml)
    || tapResource('btn-save-set', xml)
...
function trySaveOneSet() {
  ...
  if (!ensureOnWorkoutScreen()) return { ok: false, reason: 'not_on_workout' };
  skipRestIfVisible();
  ensureSimpleMode();
  focusPendingSetRow();
  for (let i = 0; i < 4; i++) scrollUp();
  let xml = uiDump();
  if (!/input-weight|Registrar \d+kg/i.test(xml)) {
    for (let s = 0; s < 10; s++) scrollDown();
    xml = uiDump();
  }
  if (!/input-weight|Registrar \d+kg|Pendente/i.test(xml)) return { ok: false, reason: 'no_input_weight' };
  fillWeightReps();
  sleep(800);
  dismissInlineKeypad();
  let tapped = false;
  for (let s = 0; s < 10; s++) {
    ...
    if (!xmlHas(xml, 'screen-workout')) {
      if (!ensureOnWorkoutScreen()) return { ok: false, reason: 'left_workout', xml };
      continue;
    }
    if (hasSaveAction(xml)) {
      tapped = tapSaveAction(xml) || route2BoundsSave(xml);
      if (tapped) break;
    }
    scrollDown();
  }
  if (!tapped) return { ok: false, reason: 'save_tap_fail', xml };
  sleep(1400);
  skipRestIfVisible();
  xml = uiDump();
  const saved = /set-saved-check|set-saved-state|Série salva|Series salvas|salva/i.test(xml)
    || /btn-finalizar-treino|Finalizar treino/i.test(xml);
  return { ok: saved, reason: saved ? 'saved_confirmed' : 'save_unconfirmed', xml };
}

function completeAllSets(maxRounds = 40) {
  let saved = 0;
  const started = Date.now();
  for (let i = 0; i < maxRounds; i++) {
    if (Date.now() - started > 600000) return { done: false, saved, reason: 'timeout_10min' };
    if (!ensureOnWorkoutScreen()) {
      ensureOnWorkoutScreen();
      sleep(1500);
    }
    const xml = uiDump();
    if (/btn-finalizar-treino/i.test(xml)) return { done: true, saved, reason: 'finish_enabled' };
    if (/Finalizar treino/i.test(xml)) return { done: true, saved, reason: 'finish_enabled' };
    ...
    if (r.reason === 'save_unconfirmed') {
      saved += 1;
      console.log('SAVE_ROUND_UNCONFIRMED', saved);
      skipRestIfVisible();
      continue;
    }
    if (r.reason === 'left_workout' || r.reason === 'not_on_workout') {
      ensureOnWorkoutScreen();
      sleep(1500);
      continue;
    }
  ...
  const tail = uiDump();
  return { done: /btn-finalizar-treino|Finalizar treino/i.test(tail), saved, reason: 'max_rounds' };
}
...
function writeManualAssistInstructions(xml) {
  ...
  const body = `# Manual assist — PR #53 Visual QA V5 (emulator-5554 ONLY)
...
function stabilizeEmulator(opts = {}) {
  const skipApk = opts.skipApkInstall || RESUME_AFTER_LOGIN || RESUME_CAPTURE;
  console.log(`SANDBOX_STABILIZE_V5: emulator-5554 only skipApk=${skipApk}`);
  wakeDevice();
  ...
  if (!skipApk) {
    ...
    console.log('APK_INSTALL_SKIPPED_TO_PRESERVE_SESSION');
  }
  launchApp();
}
...
function writeManifest(visualPass, flowMode) {
  const manifest = {
    ...
    apkInstallSkippedOnResume: RESUME_AFTER_LOGIN || RESUME_CAPTURE,
    authState: lastAuthState,
...
async function main() {
  ...
  if (RESUME_AFTER_LOGIN) {
    ...
    stabilizeEmulator({ skipApkInstall: true });
...
main().catch((err) => {
  console.error('SANDBOX_V5_ERROR', err.message);
  process.exit(1);
});
