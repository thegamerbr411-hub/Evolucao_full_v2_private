#!/usr/bin/env node
/** PR #53 — Visual QA V3 (emulator-5554 ONLY, manual-assist resume supported) */
...
const ROOT = path.join(process.cwd(), '.qa_runtime/visual_audit/hevy_ux_lote_c_v3');
...
const MANUAL_PAUSE = args.includes('--manual-assist-pause');
...
function uiDump() {
  sleep(450);
  const remote = `/sdcard/hulc3_${Date.now()}.xml`;
  for (let i = 0; i < 5; i++) {
    try {
      if (i > 0) wakeDevice();
      ...
      if (xml.includes('<hierarchy') && xml.length > 300) return xml;
    } catch (e) { /* retry */ }
  }
  return '';
}
...
function sandboxBootstrapAuth() {
  let xml = uiDump();
  if (packageOk(xml) && (/tab-home|tab-treino|screen-workout|btn_home_main_cta/i.test(xml))) return true;
  if (!/screen_login|screen-login|input_email|input-email|btn_login|btn-login|btn_go_login/i.test(xml)) {
    return packageOk(xml);
  }
  const email = process.env.QA_TEST_EMAIL || '';
  const password = process.env.QA_TEST_PASSWORD || '';
  if (!email || !password) {
    console.warn('AUTH_BOOTSTRAP_SKIP: QA_TEST_EMAIL/PASSWORD not set');
    return false;
  }
  tapResource('btn_go_login', xml) || tapPartialResource('btn_go_login', xml);
  sleep(800);
  xml = uiDump();
  ...
  sleep(400);
  xml = uiDump();
  tapResource('input_password', xml) || tapPartialResource('input-password', xml);
  sleep(400);
  ...
  for (let i = 0; i < 8; i++) {
    xml = uiDump();
    if (/tab-home|tab-treino|btn_home_main_cta|screen-workout/i.test(xml) && packageOk(xml)) return true;
    sleep(2000);
  }
  return packageOk(uiDump());
}
...
function writeManualAssistInstructions(xml) {
  ...
  const body = `# Manual assist — PR #53 Visual QA V3 (emulator-5554 ONLY)
...
6. When screen-workout-complete is visible, re-run:
   \`node .qa_runtime/scripts/hevy_ux_lote_c_sandbox_v3.cjs --resume-capture-only\`
...
function stabilizeEmulator() {
  console.log('SANDBOX_DESTRUCTIVE_ALLOWED_NOTE: emulator-5554 only');
  wakeDevice();
  adb('shell', 'am', 'force-stop', 'com.nexa.finance');
  adb('shell', 'am', 'force-stop', PKG);
  try { adb('reverse', 'tcp:8081', 'tcp:8081'); } catch (e) { /* ok */ }
  if (FRESH_SANDBOX) {
    console.log('SANDBOX_PM_CLEAR');
...
async function main() {
  ...
  stabilizeEmulator();
  let xml = ensureEvolucaoForeground();
...
main().catch((err) => {
  console.error('SANDBOX_V3_ERROR', err.message);
  process.exit(1);
});
