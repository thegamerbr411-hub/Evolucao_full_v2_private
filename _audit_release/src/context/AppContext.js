...
import { getHydrationSummaryFromApi, saveHydrationToApi, syncPendingHydration } from '../services/hydrationApiService';
...
function getTodayKey(timezone) {
  ...
  const formatter = new Intl.DateTimeFormat('en-CA', {
    timeZone: normalizeTimezone(timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC'),
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  });
...
function buildRoutineId(existingIds = new Set()) {
  let candidate = '';
  do {
    candidate = `routine-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
  } while (existingIds.has(candidate));
  return candidate;
}

function sanitizeUserRoutines(list = []) {
  ...
  if (routines.length !== source.length) {
    changed = true;
  }
...
export const AppProvider=({children})=>{
  ...
  useEffect(() => {
    ...
    loginAnonymous()
      .then(async (loggedUser) => {
        ...
        const fallbackIdentity = await getOrCreateUserIdentity();
        const resolvedUserId = String(loggedUser?.id || fallbackIdentity?.userId || '').trim();
        if (!resolvedUserId) {
          return;
        }
        ...
        await saveUserIdentity({
          ...
          source: loggedUser?.id ? 'firebase_anonymous' : 'local_fallback',
        });
      ...
      .catch(async () => {
        ...
        const fallbackIdentity = await getOrCreateUserIdentity();
        const resolvedUserId = String(fallbackIdentity?.userId || '').trim();
        if (!resolvedUserId) {
          return;
        }
  ...
  }, [workoutLogs, history, plan?.caloriesPerDay, profile?.timezone]);
  ...
  }, [gamification?.lastWorkoutDate, gamification?.streakDays, isHydrated, profile?.timezone]);
    ...
    const loadFromStorage = async () => {
      try {
        ...
        if (Array.isArray(parsed?.userRoutines)) {
          const sanitized = sanitizeUserRoutines(parsed.userRoutines);
          setUserRoutines(sanitized.routines);
        }
  ...
  useEffect(() => {
    ...
    const resolvedUserId = String(user?.id || profile?.id || '').trim();
    if (!resolvedUserId) {
      return;
    }

    const timezone = normalizeTimezone(profile?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC');

    syncPendingHydration({
      userId: resolvedUserId,
      timezone,
    })
      .then(async () => {
        const dayKey = getTodayKey(timezone);
        const summary = await getHydrationSummaryFromApi({
          userId: resolvedUserId,
          timezone,
          dayKey,
        });
        ...
        const serverWater = Number(summary?.data?.summary?.totalMl || 0);
        if (!Number.isFinite(serverWater) || serverWater < 0) {
          return;
        }

        setHistory((prev) => {
          const existing = prev.find((item) => item.date === dayKey);
          const normalizedEntry = {
            ...
            insight: existing?.insight || 'Hidratacao sincronizada com backend.',
            macroInsight: existing?.macroInsight || '',
            waterMl: Math.max(Number(existing?.waterMl || 0), serverWater),
          };

          const withoutToday = prev.filter((item) => item.date !== dayKey);
          const next = [normalizedEntry, ...withoutToday];
          return next.slice(0, 30);
        });
      })
      .catch(() => {
        // Sincronizacao falhou, fila permanece para tentativa futura.
      });
  }, [isHydrated, profile?.id, profile?.timezone, user?.id]);
  ...
  const saveDailyHistory = ({ consumedCalories, protein, carbs, fats, trainedToday, insight, macroInsight }) => {
    const entryDate = getTodayKey(profile?.timezone);
    setHistory((prev) => {
      const existingToday = prev.find((item) => item.date === entryDate);
      const normalizedEntry = {
        date: entryDate,
  ...
  const saveNutritionEntry = ({ calories, protein, carbs, fats }) => {
    ...
    setHistory((prev) => {
      ...
      previousCalories = Number(existing?.calories || 0);
      const normalizedEntry = {
        date: entryDate,
  ...
  const addWaterIntake = (ml) => {
    ...
    const resolvedUserId = String(user?.id || profile?.id || '').trim();
    const timezone = normalizeTimezone(profile?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC');
    const hydrationEventId = `${resolvedUserId || 'anonymous'}-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
    ...
    if (resolvedUserId) {
      saveHydrationToApi({
        ...
        // Fila offline garante reenvio sem bloquear UX.
      });
    }
  ...
  const getSmartWorkoutRecommendation = () => {
    ...
    const recommendation = {
      ...
      exercises: result.exercises.map((exercise, index) => ({
        ...exercise,
        id: String(exercise?.id || `smart-${exercise.name}-${index}`),
      })),
      debug: result.debug,
    };
  ...
  const getUserRoutineById = useCallback((routineId) => {
    const safeId = String(routineId || '').trim();
    if (!safeId) {
      return null;
    }

    return (Array.isArray(userRoutines) ? userRoutines : []).find((item) => String(item?.id || '').trim() === safeId) || null;
  }, [userRoutines]);
  ...
  const createUserRoutine = ({ name, frequency = 3, exercises = [] }) => {
    ...
    const existingIds = new Set((Array.isArray(userRoutines) ? userRoutines : []).map((item) => String(item?.id || '').trim()).filter(Boolean));
    const routine = {
      id: buildRoutineId(existingIds),
      name: safeName,
