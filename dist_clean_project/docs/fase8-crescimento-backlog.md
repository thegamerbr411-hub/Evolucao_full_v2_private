...
## Status de execucao (11-04-2026)

- T2 - CTA concluir rapido treino: EM PRODUCAO LOCAL.
- T1 - Baseline congelado: EM PRODUCAO LOCAL.
- T3 - Instrumentacao de variante treino: EM PRODUCAO LOCAL.
- T4 - CTA fechar dia nutricao: EM PRODUCAO LOCAL.
- T6 - Iteracao copy/hierarquia CTAs: EM PRODUCAO LOCAL.
- T7 - Segmentacao novos vs recorrentes: EM PRODUCAO LOCAL.
- T8 - Go/No-Go semana 1: EM PRODUCAO LOCAL.
- T9 - Baseline monetizacao por contexto: EM PRODUCAO LOCAL.
- T10 - Timing do paywall: EM PRODUCAO LOCAL.
- T11 - Copy do paywall: EM PRODUCAO LOCAL.
- T12 - Protocolo anti-missed_day: EM PRODUCAO LOCAL.
- T13 - Reforco de streak_updated: EM PRODUCAO LOCAL.
- T14 - Consolidacao/review executivo: EM PRODUCAO LOCAL.
- Demais tickets: PENDENTES.
...
- Owner sugerido:
  - Produto + Engenharia.
- Status inicial:
  - EM PRODUCAO LOCAL (baseline automatizado em `artifacts/growth-go-no-go.json`).
...
- Owner sugerido:
  - Engenharia mobile.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando coleta de 72h).
...
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando validacao de cobertura >=95%).
...
- Owner sugerido:
  - Produto.
- Status inicial:
  - EM PRODUCAO LOCAL (checklist automatizado em `artifacts/growth-go-no-go.json`).
...
- Owner sugerido:
  - Produto + Design.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando leitura de CTR em 72h).
...
- Owner sugerido:
  - Produto analytics.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando comparativo por coorte).
...
- Owner sugerido:
  - Produto + Engenharia.
- Status inicial:
  - EM PRODUCAO LOCAL (decisao automatizada em `artifacts/growth-go-no-go.json`).
...
- Owner sugerido:
  - Produto growth.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando volume minimo por source para leitura).
...
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando leitura de view->click em 72h).
...
- Owner sugerido:
  - Produto + Conteudo.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando leitura de click->convert em 72h).
...
- Owner sugerido:
  - Engenharia + Produto.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando leitura de reducao >=8%).
...
- Owner sugerido:
  - Produto + Engenharia.
- Status inicial:
  - EM PRODUCAO LOCAL (aguardando leitura de engajamento D+1).
...
- Owner sugerido:
  - Produto.
- Status inicial:
  - EM PRODUCAO LOCAL (relatorio em `artifacts/growth-exec-review.md`).
