...
async function launchApp({ deleteApp = false } = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      if (isAttachedRun) {
        const forceNewInstance = Boolean(deleteApp);
        ...
        // Em attached, preferir reuse para evitar reconexão de sessão Detox entre specs.
        await device.launchApp({ newInstance: forceNewInstance });
      } else {
        try {
          await device.terminateApp();
        } catch {
